...
async def remove_redeem_code(redeem_code, user_redeem):
    ...
    async with aiofiles.open("users/redeem.txt", mode="w+", encoding="utf-8") as redeem_file:
...
def register(app):
    ...
    async def redeem_cmd(app_client, message):
        async with aiofiles.open("users/redeem.txt", mode="r+", encoding="utf-8") as redeem_file:
        ...
        if approved:
            async with aiofiles.open(file="users/premium.txt", mode="r+", encoding="utf-8") as premium_existed:
                existed_user = await premium_existed.read()
                if str(message.from_user.id) + '\n' not in existed_user:
                    async with aiofiles.open("users/alredeem.txt", mode="w+", encoding="utf-8") as al_redeem:
