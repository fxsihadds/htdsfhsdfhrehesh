                ...
                if 'idToken' in auth_data:
                    ...
                    if subscription_response.status_code == 200:
                        ...
                        if 'plan' in subscription_data:
                            plan_name = subscription_data['planMetadata'].get('name')
                            premium_accounts.append(f"{combo} [Plan Name: {plan_name}]")
                            success_count += 1
...
def register(app):
    @app.on_message(filters.command("chaupal"))
    async def chaupal_command(_, message):
        await chaupal_helper_command(app, message)
